...
import com.lantalk.CommonSoundClass;
import com.lantalk.Playback;
...
public class ChatClient extends JFrame implements Runnable, ActionListener{
    ...
    private String connectAddr = "192.168.1.227";//172.16.147.76";
...
//    int alternatenumber = 9999;
    ...
    //i.e among noise how it will find that it s data
    ...
    // yet to be implemented on the client side
    ...
    // Variables declaration - do not modify this is for applet part
    ...
    //  Should reuse content from both constructors since largely the same
    public ChatClient(String connectIPAddr) {
        connectAddr = connectIPAddr;

        prepareUI(); //prepares user interface
    }
    ...
    private void lstNickMouseReleased(java.awt.event.MouseEvent evt) {
...
//yet to be added
    private void mnuMuteActionPerformed(java.awt.event.ActionEvent evt) {
        try{
    ...
    private void lstNickKeyReleased(java.awt.event.KeyEvent evt) {
        KeyWasReleased( evt);
    }

    private void txtOutputKeyReleased(java.awt.event.KeyEvent evt) {
        KeyWasReleased( evt);
    }

    private void chkHandsFreeKeyReleased(java.awt.event.KeyEvent evt) {
        KeyWasReleased( evt);
    }

    private void btnTalkKeyReleased(java.awt.event.KeyEvent evt) {
        KeyWasReleased( evt);
    }
    ...
    private void chkHandsFreeKeyPressed(java.awt.event.KeyEvent evt) {KeyWasPressed( evt.getKeyCode() );
    }

    private void lstNickKeyPressed(java.awt.event.KeyEvent evt) {
        KeyWasPressed( evt.getKeyCode() );
    }

    private void txtOutputKeyPressed(java.awt.event.KeyEvent evt) {
        KeyWasPressed( evt.getKeyCode() );
    }

    private void btnTalkKeyPressed(java.awt.event.KeyEvent evt) {
        KeyWasPressed( evt.getKeyCode() );
    }
    ...
    private void chkHandsFreeActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    */
    ...
    /**
     * common sound class queue for recieved data, so the in.read won't hang during calculations.
     */
...
//may run into prob for large text messages
                        String passedObj = "";
...
//                            int packetNumber = bytepassedObj[sizeread - 1];
     ...
     *If you  want to write a program that will use this to create list of people available for chat 
     *You can pass all these thigns to its child processes.i.e a class(thread )inheriting this class 
     */
    public void run() {
        try{
...
//hands free mode is quite similar to what used to be in mobile phones
            byte[] b2 = null;
    ...
    public byte[] removeNoise(byte[] b ){ // doesnt really remove noise i use this for hands free mode
		...
		*Is used for hands free recording mode doesnt remove noise
                * we need to work over removing noise and moreover on repetition of sound (echo)
		*/
    ...
    /**
     *Decompresses the data daata can't be sent in its raw form as it is very heavy so decompressing lowers its wt
     */
